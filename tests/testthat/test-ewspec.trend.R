...
test_that("Supplied matrix is square", {
  skip_on_cran()
  x <- stats::rnorm(256)
  expect_error(ewspec.trend(x, supply.mat = TRUE, mat = matrix(0,7,8)),
...
test_that("Supplied matrix is large enough", {
  ...
  x <- stats::rnorm(256)
  A <- Cmat.calc(J=4)
  expect_error(ewspec.trend(x, supply.mat = TRUE, mat = solve(A), max.scale = 6),
...
test_that("binwidth is integer", {
  expect_error(ewspec.trend(rnorm(64), binwidth = 9.7),
               "binwidth parameter must be an integer.")
})

test_that("max.scale is integer", {
  expect_error(ewspec.trend(rnorm(64), max.scale= 3.7),
               "max.scale parameter must be an integer.")
})

test_that("max.scale isn't too large", {
  expect_warning(ewspec.trend(rnorm(64), max.scale = 9),

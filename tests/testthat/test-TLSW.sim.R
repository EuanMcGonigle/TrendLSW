...
for (i in seq_len(length(dists))) {
  test_that(paste0("TLSW.sim with distribution #", i, " executes"), {
    skip_on_cran()
    expect_equal(class(TLSW.sim(spec = spec, distribution = dists[i])), "numeric")
  })
}

test_that("distribution is recognised", {
  expect_error(
    TLSW.sim(spec = spec, distribution = "cauchy"),
...
test_that("df is bigger than 2 for t distribution", {
  expect_error(
    TLSW.sim(spec = spec, df = 1, distribution = "t"),

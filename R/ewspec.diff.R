ewspec.diff = function(data, lag=1, filter.number  = 1, family = "DaubExPhase",
  ...
  calc.final.spec = function(spec, dyadic,data.len){
    ...
    if(dyadic==TRUE){
      final_spec = wavethresh::cns(2^(spec$nlevels-2), filter.number = spec$filter$filter.number,
      ...
      final_spec = wavethresh::cns(2^final.spec.J, filter.number = spec$filter$filter.number,
                       family = spec$filter$family)
